...
# Define a mapping of runs to configurations
run_to_config = {
    1: "1 (PoS)",
    3: r"$\lambda = 0.8$",
    5: r"$\lambda = 0.6$",
...
def process_csv_file(csv_file_path):
    """Reads the CSV file, filters the desired runs, and calculates the desired values."""
    df = pd.read_csv(csv_file_path)

    # Drop rows with NaN in the 'runs' column
    df_clean = df.dropna(subset=["runs"])

    # Filter for only the desired runs
    df_filtered = df_clean[df_clean["runs"].isin(run_to_config.keys())]

    # Group by 'runs' and aggregate directly
    df_grouped = (
        df_filtered.groupby("runs")
        .agg(consensus_tps=("consensus_tps", "max"), consensus_latency=("consensus_latency", "min"))
        .reset_index()
    )

    # Map the runs to their corresponding configurations
    df_grouped["config"] = df_grouped["runs"].map(run_to_config)
...
def plot_combined(all_data_hotstuff, all_data_cometbft, output_folder):
    ...
    num_lambda = 6  # Number of selected lambda values
    ...
    for j in range(num_lambda):
        axs[0, 0].bar(
            x_hotstuff + j * bar_width,
            bar_positions_tps_hotstuff[j],
            width=bar_width,
            label=list(run_to_config.values())[j],
    ...
    for j in range(num_lambda):
        axs[0, 1].bar(
            x_cometbft + j * bar_width,
            bar_positions_tps_cometbft[j],
            width=bar_width,
            label=list(run_to_config.values())[j],
            color=color_palette[j % len(color_palette)],
        )
    ...
    for j in range(num_lambda):
        axs[1, 0].bar(
            x_hotstuff + j * bar_width,
            bar_positions_latency_hotstuff[j],
            width=bar_width,
            label=list(run_to_config.values())[j],
            color=color_palette[j % len(color_palette)],
        )
    ...
    for j in range(num_lambda):
        axs[1, 1].bar(
            x_cometbft + j * bar_width,
            bar_positions_latency_cometbft[j],
            width=bar_width,
            label=list(run_to_config.values())[j],
            color=color_palette[j % len(color_palette)],
        )
